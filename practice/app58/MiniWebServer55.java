  ...
  public static void main(String[] args) throws Exception {
    ...
    for (Class<?> servlet : servlets) {
      ...
      if(params.length == 0) { // 생성자의 파라미터가 없다면
        objMap.put(servletPath, constructor.newInstance()); // 생성자를 가지고 인스턴스를 만들어라
        //System.out.println(servlet.getName());
      } else { // 생성자의 파라미터가 있다면
        // 그 파라미터 타임과 일치하는 객체를 찾아서 그 객체를 가지고 생성자를 호출한다.
    ...
    //      objMap.put(servletPath, constructor.newInstance(boardDao)); // 인스턴스를 생성해라....
    //
    //    } else if (params[0].getType() == MemberDao.class){
    //      objMap.put(servletPath, constructor.newInstance(memberDao));
    //
    //    }
