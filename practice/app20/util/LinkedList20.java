...
public class LinkedList {
  ...
  public void append(Object value) {
    ...
    // 만약, 리스트에 노드가 없다면
    if (tail == null) {
      head = tail = node; // 첫 노드를 등록한다. (tail = node)를 먼저 실행
      return;
    }
  ...
  public Object delete(int index) {
    ...
    if (cursor.prev != null) { // 맨 앞 노드가 아니라면
      ...
      head = cursor.next; // 삭제할 다음 노드를 시작 노드를 가리키게 한다.
    ...
    if (cursor.next != null) { // 마지막 노드가 아니라면
      ...
      tail = cursor.prev; // 혅재 커서의 이전 노드를 마지막 노드로 설정한다.
      tail.next = null; // 마지막 노드이기 때문에 다음 노드를 가리키지 않게 한다.
    }

    // 삭제할 노드를 초기화시킨다.
    // => garbage 객체가 살아 있는 객체를 참조하지 않게 한다.
    // => garbage 객체가 다른 garbage 객체를 참조하지 않게 한다.
    deleted = cursor.value; // 노드를 삭제하기 전에 노드에 들어있는 값을 임시 보관해 둔다.
    cursor.value = null;
    cursor.prev = null;
    cursor.next = null;
  ...
  public int length() { // size 변수를 다른 곳에서 변경할 수 없지만 사용할 수 있게 하기 위해 만듬
    return size;
  }
